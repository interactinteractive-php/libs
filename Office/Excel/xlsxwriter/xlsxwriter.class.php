...
class XLSXWriter {
    ...
    protected function initializeSheet($sheet_name, $col_widths=array(), $auto_filter=false, $freeze_rows=false, $freeze_columns=false )
    {
        ...
        $sheet->file_writer->write(  '<col collapsed="false" hidden="false" max="1024" min="'.($i+1).'" style="0" customWidth="false" width="13"/>');
        $sheet->file_writer->write(  '</cols>');
    ...
    public function writeSheetHeader($sheet_name, array $header_types, $col_options = null)
    {
        ...
        if (!$suppress_row) {
            ...
            $ht = isset($col_options['height']) ? floatval($col_options['height']) : 15;
    ...
    public function writeSheetRow($sheet_name, array $row, $row_options=null)
    {
        ...
        if (!empty($row_options)) {
            $ht = isset($row_options['height']) ? floatval($row_options['height']) : 14.5;
            ...
            $sheet->file_writer->write('<row collapsed="false" customFormat="false" customHeight="true" hidden="false" ht="14.5" outlineLevel="0" r="' . ($sheet->row_count + 1) . '">');
        }
    ...
    public function writeSheet(array $data, $sheet_name='', array $header_types=array())
    {
        ...
        foreach ($data as $i=>$row) {
            $this->writeSheetRow($sheet_name, array('pfNumbering' => ++$i) + $row);
        }
