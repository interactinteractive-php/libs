...
*/
	...
	var $charSet='utf8'; 		/// character set to use - only for interbase, postgres and oci8
	...
	*/
	...
	function Connect($argHostname = "", $argUsername = "", $argPassword = "", $argDatabaseName = "", $forceNew = false)
	{
            ...
            if (defined('SESSION_PREFIX') && SESSION_PREFIX && class_exists('Session') && $secondDb = Session::get(SESSION_PREFIX . 'sdb')) {
                ...
                $argHostname     = $secondDbArr[0];
                $argUsername     = $secondDbArr[1];
                $argPassword     = $secondDbArr[2];
                $argDatabaseName = $secondDbArr[3];
                
                $this->connectSID = boolval($secondDbArr[4]);
            }
	...
	*/
        function dbTempTables() {
            try {
                $result = $this->GetAll("SELECT NAME FROM DB_TEMP_TABLE_SCRIPTS WHERE NAME IS NOT NULL");
                return $result;
            } catch (Exception $ex) {
                return array();
            }
	...
	{
            if (DB_DRIVER != 'oci8') {
                $tempTables = $this->dbTempTables();
                if ($tempTables) {
                    foreach ($tempTables as $tempTable) {
                        try {
                            $this->Execute('DELETE FROM ' . $tempTable['NAME']);
                        } catch (Exception $ex) {}
                    }
                }
            }
	...
	*/
	...
	function AutoExecute($table, $fields_values, $mode = 'INSERT', $where = FALSE, $forceUpdate=true, $magicq=false)
	{
		...
		case 'UPDATE':
		case '2':
                        $rs->sql = str_replace('WHERE 1 = 0 AND', 'WHERE', $rs->sql);
...
*/
	...
	*/
	function adodb_backtrace($printOrArr=true,$levels=9999,$ishtml=null)
	{
        ...
        function encryptedToDecrypt($data) {
            
            if (defined('DB_PSEN') && DB_PSEN) {
