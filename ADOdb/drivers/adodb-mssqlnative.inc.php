...
class ADODB_mssqlnative extends ADOConnection {
	...
	{
		return " CASE $field
                            WHEN null THEN $ifNull
                            ELSE $ifNotNull
                            END "; // if MS SQL Server
	}
        
        function listAgg( $field, $symbol, $sortOrder = null, $tableName = null, $aliasFromTable = '' )
        {
            $fieldName = "";
            $tableNameLittle = "";
            $array = explode(".", $field);
            if(count($array) == 1){
                ...
                $fieldName = $array[1];
                $tableNameLittle = $array[0];
            }
            
            if ($aliasFromTable != '') {
                $aliasFromTable = $aliasFromTable . '.';
            }
            
            return " STUFF((SELECT distinct '" . $symbol . "' + CONVERT(varchar(max), " . $aliasFromTable . $field . ") FROM " . $tableName . " TMP_LISTAGG_TABLE WHERE " . $aliasFromTable . $field . " = TMP_LISTAGG_TABLE." . $fieldName . " FOR XML PATH ('')) , 1, 1, '') ";		
	}
        
        function trim( $field )
	{
		return " LTRIM(RTRIM($field)) "; // if Oracle
	}
        
        function groupBy($fileds1, $fileds2) 
        {
            return " " . $fileds2 . ", " . $fileds1 . " ";
        }
	...
	{
            return " CONVERT(DATETIME, $field, 102) "; // toDate Oracle
	}
	...
	*/
	function UpdateBlob($table,$column,$val,$where,$blobtype='BLOB')
	{
		...
		if (strtoupper($blobtype) == 'CLOB') {
			$sql = "UPDATE $table SET $column=N'" . $val . "' WHERE $where";
	...
	function MetaColumns($table, $upper=true, $schema=false){
        ...
        function getConcatOperator()
	{
		return $this->concat_operator;
	}
