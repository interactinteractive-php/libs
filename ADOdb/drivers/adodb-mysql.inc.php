  ...
  function SetCharSet($charset_name)
  {
	...
	function IfNull( $field, $ifNull )
	{
        ...
        {            
            ...
            return " GROUP_CONCAT(" . $field . " " . $sortOrderString . " separator '" . $symbol . "' ";		
	}
        
        function trim( $field )
	{
		return " TRIM($field) "; // if Oracle
	}
        ...
        function ToDate( $field, $format = '%Y-%m-%d' )
	{
		return " DATE_FORMAT($field, '$format') "; // toDate MySQL
	}
...
//		return "from_unixtime(unix_timestamp($date)+$fraction)";
	...
	function SelectLimit($sql,$nrows=-1,$offset=-1,$inputarr=false,$secs=0)
	{
		...
		if ($secs)
                    $rs = $this->CacheGetAll($secs,$sql." LIMIT $offsetStr".((integer)$nrows),$inputarr);
		else    
                    $rs = $this->Execute($sql." LIMIT $offsetStr".((integer)$nrows),$inputarr);
                    if (isset($rs->_array)) {
                        return $rs;
                    } else {
                        $data = $this->GetAll($sql." LIMIT $offsetStr".((integer)$nrows),$inputarr);
                        $rs->_array = $data;
                        return $rs; //(object) array_merge((array) $rs, (array) array('_array' => (array) $data));  
                    }
	}
