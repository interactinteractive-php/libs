 ...
 * @category	Format
 * @author	B.Och-Erdene
 * @link	http://www.interactive.mn/PHPframework/Format
 */
...
class Format {
    ...
    public function valueFormatting($value, $inputType) {
        switch (strtolower($inputType)) {
            ...
            case 'boolean':
                ...
                break;    
            case 'signature':
                return '<img src="data:image/jpeg;base64,'.$value.'" style="width: 215px;">';
                break;
            case 'text_editor':
    ...
    public static function dataGridGroupFormatter($formatterStr) {
         
        if ($formatterStr) {
            
            $formatterStr = str_replace('[value]', "'+value+'", $formatterStr);
            $formatterStr = str_replace('[length]', "'+rows.length+'", $formatterStr);
    ...
    public static function xml_pretty($xmlString) {
